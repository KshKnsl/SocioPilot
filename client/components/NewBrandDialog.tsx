...
export default function NewBrandDialog({ open, setOpen, onCreated }: { open: boolean; setOpen: (v: boolean) => void; onCreated?: () => void }) {
  ...
  async function onCreate() {
    ...
    try {
      ...
      if (res?._id) { localStorage.setItem("sp_selected_brand", res._id); window.dispatchEvent(new Event("storage")); }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-125">
        ...
        <div className="grid gap-6 py-4">
          <div className="space-y-2"><Label>Brand Name</Label><Input placeholder="e.g. TechFlow" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} /></div>
          <div className="space-y-2"><Label>Description</Label><Textarea placeholder="What does your brand do?" className="min-h-25" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} /></div>
          <div className="space-y-3"><Label>Writing Styles</Label>
            <ScrollArea className="h-37.5 w-full rounded-md border p-4"><div className="flex flex-wrap gap-2">
              {writingStyles.map(s => (
                <Badge key={s} variant={form.styles.includes(s) ? "default" : "outline"} className="cursor-pointer px-3 py-1" onClick={() => toggleStyle(s)}>{s}</Badge>
              ))}
            </div></ScrollArea>
          </div>
        </div>
        <DialogFooter>
          ...
          <Button onClick={onCreate} disabled={loading || !form.title || !form.description} className="bg-primary">
